<template>
    ...
    <div style="align-items: center;">
      <fwb-modal v-if="isShowModal" @close="closeModal" class="max-w-2xl" position="center">
        ...
        <fwb-input v-model="tekananDarah" label="Tekanan Darah" placeholder="Tekanan Darah" size="sm" />
        <fwb-input v-model="beratBadan" label="Berat Badan" placeholder="Berat Badan" size="sm" />
        </template>
        <template #footer>
...
import {
  ...
  try {
      ...
      const response = await fetch.post(
        "http://localhost:8000/api/" + 'post-keperawatan',
        payload
      );
      if (response.success) {
        closeModal();
